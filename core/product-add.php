<?php
    ...
    if(isset($_POST['name']) && isset($_POST['quantity']) && isset($_POST['supplier_price']) && isset($_POST['sale_price']) && isset($_POST['voucher_no']) && isset($_POST['supplier_id']) && isset($_POST['warranty_days'])){
        ...
        if(!empty($name) && !empty($quantity) && !empty($quantity) && !empty($supplier_price) && !empty($sale_price) && !empty($voucher_no) && !empty($supplier_id) && !empty($warranty_days)){
            $sql = "INSERT INTO `products`(`name`, `quantity`, `supplier_price`, `sale_price`, `voucher_no`, `supplier_id`, `warranty_days`) VALUES ('$name','$quantity', '$supplier_price', '$sale_price', '$voucher_no', '$supplier_id', '$warranty_days')";
            // die($sql);
