<?php
    ...
    if(isset($_POST['product_edit_id']) && isset($_POST['name']) && isset($_POST['quantity']) && isset($_POST['supplier_price']) && isset($_POST['sale_price']) && isset($_POST['voucher_no']) && isset($_POST['supplier_id']) && isset($_POST['warranty_days']) && isset($_POST['product_details'])){
        ...
        if(!empty($product_edit_id) && !empty($name) && !empty($quantity) && !empty($supplier_price) && !empty($sale_price) && !empty($voucher_no) && !empty($supplier_id) && !empty($warranty_days) && !empty($product_details)){
            $sql = "UPDATE `products` SET `name` = '$name', `quantity` = '$quantity', `supplier_price` = '$supplier_price', `sale_price` = '$sale_price', `voucher_no` = '$voucher_no', `supplier_id` = '$supplier_id', `warranty_days` = '$warranty_days', `product_details` = '$product_details' WHERE `id` = '$product_edit_id'";
            // die($sql);
